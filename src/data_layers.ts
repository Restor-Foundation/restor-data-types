...
export interface IDataLayerPropertyTime {
  ...
  shaderUniform?: boolean;
  // Set to true if a range can be selected. Default is only a single point
  range?: boolean;
  /** (Optional) List of palette colors to display in color range.
   * Colors should start with # */
  palette?: string[];
...
export type IDataLayerProperty =
  | IDataLayerPropertyRange
  | IDataLayerPropertyTime
  | IDataLayerPropertySwitch
  | IDataLayerPropertyCategories;
